use adnl::{
    common::{
        add_object_to_map, add_object_to_map_with_update, deserialize, get256, hash, hash_boxed,
        serialize, serialize_inplace, AdnlPeers, KeyId, KeyOption, Query, QueryResult, Subscriber,
        Version, Wait,
    },
...
use ton_api::{
    ton::{
        self,
        adnl::{addresslist::AddressList, AddressList as AddressListBoxed},
        dht::{
            key::Key as DhtKey,
        ...
        overlay::{
            node::Node as OverlayNode, nodes::Nodes as OverlayNodes, Nodes as OverlayNodesBoxed,
        },
...
impl DhtNode {
    ...
    pub fn add_peer(&self, peer: &Node) -> Result<Option<Arc<KeyId>>> {
        ...
        if self.known_peers.put(ret.clone())? {
            ...
            add_object_to_map(&self.buckets, affinity, || Ok(lockfree::map::Map::new()))?;
            if let Some(bucket) = self.buckets.get(&affinity) {
                add_object_to_map_with_update(bucket.val(), ret.clone(), |old_node| {
    ...
    pub async fn ping(&self, dst: &Arc<KeyId>) -> Result<bool> {
        ...
        let query = TLObject::new(DhtPing { random_id });
    ...
    async fn find_value(
        ...
        let mut ret = Vec::new();
        let query = TLObject::new(FindValue {
    ...
    fn process_store_overlay_nodes(&self, dht_key_id: DhtKeyId, value: DhtValue) -> Result<bool> {
        ...
        add_object_to_map_with_update(&self.storage, dht_key_id, |old_value| {
